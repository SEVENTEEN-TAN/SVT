# SVT 系统架构文档

## 文档信息

| 项目 | 内容 |
|------|------|
| **项目名称** | SVT (Seventeen) 风险管理系统 |
| **文档版本** | v1.0.0 |
| **文档日期** | 2025-11-17 |
| **架构师** | Winston |
| **文档类型** | Brownfield Architecture (现有系统架构) |

---

## 📋 文档导航

1. [项目概述](#一项目概述)
2. [架构设计](#二架构设计)
3. [后端架构](#三后端架构-svt-server)
4. [前端架构](#四前端架构-svt-web)
5. [安全架构](#五安全架构)
6. [数据架构](#六数据架构)
7. [部署架构](#七部署架构)
8. [性能优化](#八性能优化)
9. [技术债务](#九技术债务)
10. [未来规划](#十未来规划)

---

## 一、项目概述

### 1.1 项目简介

SVT (Seventeen) 是一个**企业级风险管理系统**，采用前后端分离架构，基于现代化技术栈构建。系统具备完整的用户管理、权限控制、审计日志等企业级特性。

**核心特性：**
- ✅ JWT智能续期认证
- ✅ AES-256 API加密
- ✅ 数据库分布式锁
- ✅ 分布式ID生成
- ✅ 动态路由权限
- ✅ 模块化布局系统
- ✅ 智能Tab管理

### 1.2 技术栈总览

| 层级 | 技术选型 | 版本 |
|------|----------|------|
| **后端框架** | Spring Boot | 3.5.7 |
| **编程语言** | Java | 21 (LTS) |
| **ORM框架** | MyBatis-Flex | 1.10.9 |
| **数据库** | SQL Server | 2019+ |
| **本地缓存** | Caffeine | 3.1.8 |
| **前端框架** | React | 19.1.0 |
| **编程语言** | TypeScript | 5.8.3 |
| **构建工具** | Vite | 6.3.5 |
| **UI组件库** | Ant Design | 5.25.4 |
| **状态管理** | Zustand | 5.0.5 |

### 1.3 系统特点

**架构优势：**
1. **职责分离明确**: 前后端完全分离，各层职责清晰
2. **注解驱动开发**: 使用自定义注解简化开发
3. **本地缓存优先**: 使用Caffeine替代Redis，简化部署
4. **类型安全**: 100% TypeScript类型覆盖
5. **性能优化**: 代码分割、懒加载、O(1)权限检查

**安全特性：**
1. **多层加密**: API加密(AES-256) + 配置加密(SM4) + 密码哈希(Argon2)
2. **九步认证**: JWT九步安全检查流程
3. **四层防护**: 认证 → 角色选择 → 状态验证 → 权限检查
4. **智能续期**: 基于用户活跃度的JWT智能续期

---

## 二、架构设计

### 2.1 整体架构

```
┌─────────────────────────────────────────────────────────────┐
│                        客户端层                              │
│  React 19.1.0 + TypeScript 5.8.3 + Ant Design 5.25.4       │
│  ┌───────────┐ ┌───────────┐ ┌───────────┐ ┌──────────┐   │
│  │  Layout   │ │  Router   │ │  Stores   │ │  Pages   │   │
│  │  System   │ │  System   │ │  (Zustand)│ │          │   │
│  └───────────┘ └───────────┘ └───────────┘ └──────────┘   │
└─────────────────────────────────────────────────────────────┘
                            │
                   HTTPS + AES-256加密
                            │
┌─────────────────────────────────────────────────────────────┐
│                      网关/代理层                             │
│            Nginx (反向代理 + Session Sticky)                │
└─────────────────────────────────────────────────────────────┘
                            │
┌─────────────────────────────────────────────────────────────┐
│                       应用服务层                             │
│  Spring Boot 3.5.7 + Java 21 + MyBatis-Flex 1.10.9         │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐     │
│  │  过滤器  │ │ AOP切面  │ │ 框架层   │ │ 业务层   │     │
│  │  (Filter)│ │ (Aspect) │ │ (Frame)  │ │ (Module) │     │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘     │
└─────────────────────────────────────────────────────────────┘
                            │
┌─────────────────────────────────────────────────────────────┐
│                       数据持久层                             │
│    SQL Server 2019+ (主数据库 + 分布式锁 + 分布式ID)        │
│    Caffeine Cache (本地缓存: JWT + 用户详情 + ID批量)       │
└─────────────────────────────────────────────────────────────┘
```

### 2.2 分层架构

#### 2.2.1 前端分层

```
┌─────────────────────────────────────────┐
│         展示层 (Presentation)            │
│  Pages / Components / Layout            │
└─────────────────────────────────────────┘
                  ↓
┌─────────────────────────────────────────┐
│           路由层 (Router)                │
│  ProtectedRoute + DynamicPage           │
└─────────────────────────────────────────┘
                  ↓
┌─────────────────────────────────────────┐
│         状态层 (State Management)        │
│  Zustand (auth/user) + React Query      │
└─────────────────────────────────────────┘
                  ↓
┌─────────────────────────────────────────┐
│          服务层 (API Service)            │
│  axios + 加密/解密 + 拦截器              │
└─────────────────────────────────────────┘
```

#### 2.2.2 后端分层

```
┌─────────────────────────────────────────┐
│        控制器层 (Controller)             │
│  接收请求 + 参数校验 + 返回响应          │
└─────────────────────────────────────────┘
                  ↓
┌─────────────────────────────────────────┐
│         业务层 (Service)                 │
│  业务逻辑 + 事务管理 + 分布式锁          │
└─────────────────────────────────────────┘
                  ↓
┌─────────────────────────────────────────┐
│       数据访问层 (Mapper/DAO)            │
│  MyBatis-Flex + SQL执行 + 结果映射      │
└─────────────────────────────────────────┘
                  ↓
┌─────────────────────────────────────────┐
│         数据持久层 (Database)            │
│  SQL Server + 分布式锁表 + ID生成表      │
└─────────────────────────────────────────┘
```

### 2.3 核心设计模式

#### 2.3.1 注解驱动开发 (Annotation-Driven)

```java
// 使用注解简化开发
@PostMapping("/create")
@Audit(description = "创建用户", recordParams = true)      // 审计日志
@RequiresPermission("user:create")                       // 权限验证
@AutoTransaction(type = TransactionType.REQUIRED)        // 自动事务
public Result<String> createUser(@RequestBody UserDTO dto) {
    return Result.success(userService.createUser(dto));
}
```

**AOP切面自动处理：**
- 审计日志记录
- 权限验证
- 事务管理
- 参数脱敏

#### 2.3.2 监听器模式 (Listener Pattern)

```java
@Table(value = "user_info",
       onInsert = FlexInsertListener.class,    // 插入监听
       onUpdate = FlexUpdateListener.class)    // 更新监听
public class UserInfo {
    @DistributedId(prefix = "U")               // 自动生成ID
    private String userId;

    @AutoFill(type = FillType.USER_ID, operation = INSERT)  // 自动填充
    private String createBy;
}
```

**监听器自动处理：**
- 分布式ID生成
- 审计字段自动填充 (createBy, createTime, updateBy, updateTime)

#### 2.3.3 缓存策略 (Caching Strategy)

```
请求流程：
1. 检查Caffeine本地缓存
   ├─ 命中 → 直接返回
   └─ 未命中 → 查询数据库 → 写入缓存 → 返回
```

**缓存使用场景：**
- JWT Token缓存 (最多1000个，30分钟过期)
- 用户详情缓存 (提升权限验证性能)
- 分布式ID批量缓存 (减少数据库访问)

---

## 三、后端架构 (SVT-Server)

### 3.1 整体结构

```
common (通用层)
    ├── annotation (自定义注解)
    ├── config (配置类)
    ├── filter (过滤器)
    └── util (工具类)
        ↓
frame (框架层)
    ├── aspect (AOP切面)
    ├── cache (缓存管理)
    ├── security (安全框架)
    ├── lock (分布式锁)
    └── dbkey (分布式ID)
        ↓
modules (业务层)
    └── system (系统管理模块)
        ├── controller (控制器)
        ├── service (业务逻辑)
        ├── entity (实体类)
        ├── dto (数据传输对象)
        └── mapper (数据访问)
```

### 3.2 过滤器链 (Filter Chain)

**执行顺序 (Order越小越先执行):**

```
1. AESCryptoFilter (Order=10)
   ├─ 请求解密 (检查X-Encrypted头)
   └─ 响应加密

2. RequestWrapperFilter (Order=50)
   └─ 请求包装 (支持多次读取Body)

3. JwtAuthenticationFilter
   └─ JWT九步安全检查
```

### 3.3 AOP切面 (Aspect)

**切面执行顺序:**

```
1. AutoTransactionAspect (Order=100)
   └─ 事务管理

2. PermissionAspect
   └─ 权限验证

3. AuditAspect
   └─ 审计日志记录

4. 目标方法执行
```

### 3.4 核心框架组件

#### 3.4.1 JWT认证流程

```
客户端请求
    ↓
1. 提取Token (从Authorization头)
   ↓
2. 验证Token格式和签名
   ↓
3. 九步安全检查
   ├─ 验证是否系统颁发的合法Token
   ├─ 检查是否在黑名单中
   ├─ 检查JWT缓存是否存在
   ├─ 检查IP地址变化
   ├─ 检查Token变化 (单点登录)
   ├─ 会话活跃度过期检查
   ├─ 智能活跃度续期检查
   ├─ 会话状态计算和响应头设置
   └─ 更新用户最后活动时间
   ↓
4. 通过验证，放行请求
```

**关键文件**: `frame/security/filter/JwtAuthenticationFilter.java:245`

#### 3.4.2 分布式锁实现

**数据库分布式锁原理:**

```sql
-- 利用主键唯一性实现互斥
CREATE TABLE distributed_lock (
    lock_key VARCHAR(255) PRIMARY KEY,  -- 唯一约束保证互斥
    lock_value VARCHAR(255),
    expire_time DATETIME
);

-- 获取锁：尝试插入
INSERT INTO distributed_lock (lock_key, lock_value, expire_time)
VALUES ('user:create:admin', 'uuid-xxx', DATEADD(SECOND, 10, GETDATE()));

-- 主键冲突 → 获取锁失败
-- 插入成功 → 获取锁成功
```

**智能重试机制:**
1. 检测到锁被占用
2. 等待指定间隔后重试
3. 累计重试次数
4. 达到最大重试次数后强制释放锁 (可配置)

**关键文件**: `frame/lock/DatabaseDistributedLockManager.java`

#### 3.4.3 分布式ID生成

**ID格式:** `前缀 + 日期 + 序号 + 字母扩展`

```
示例:
U20250617000001    # 用户ID
M20250617000002    # 菜单ID
R20250617000003A   # 角色ID (序号超999999后加字母)
```

**生成流程:**
1. 从缓存获取ID (批量预分配100个)
2. 缓存未命中 → 获取分布式锁
3. 检查日期变更 → 重置序号
4. 生成一批ID写入缓存
5. 释放锁，返回第一个ID

**关键文件**: `frame/dbkey/DistributedIdGenerator.java`

### 3.5 缓存架构

**Caffeine缓存配置:**

```java
// JWT Token缓存
Caffeine.newBuilder()
    .maximumSize(1000)              // 最多1000个Token
    .expireAfterWrite(30, MINUTES)  // 30分钟过期
    .recordStats()                  // 记录统计
    .build();
```

**缓存工具类家族:**

| 缓存工具 | 用途 | 容量 | 过期时间 |
|---------|------|------|----------|
| JwtCacheUtils | JWT Token | 1000 | 30分钟 |
| UserDetailCacheUtils | 用户详情 | 1000 | 30分钟 |
| DbKeyCacheUtils | 分布式ID批量 | 无限 | 不过期 |
| OrgInfoCacheUtils | 组织机构 | 500 | 1小时 |

**缓存策略:**
- ✅ 本地缓存优先 (简化部署)
- ✅ Session Sticky (确保用户请求路由到同一实例)
- ✅ 服务重启后用户需重新登录 (安全设计)

---

## 四、前端架构 (SVT-Web)

### 4.1 整体结构

```
utils (工具层)
    ├── request.ts (HTTP客户端)
    ├── tokenManager.ts (Token管理)
    ├── sessionManager.ts (会话管理)
    └── crypto.ts (加密工具)
        ↓
stores (状态管理)
    ├── authStore.ts (认证状态)
    ├── userStore.ts (用户状态)
    └── useAuth.ts (组合Hook)
        ↓
hooks (自定义Hooks)
    ├── useUserStatus.ts
    └── useMobile.ts
        ↓
api (API服务层)
    └── system/ (系统管理API)
        ↓
components (组件层)
    ├── Layout/ (布局系统)
    └── DynamicPage/ (动态页面)
        ↓
pages (页面层)
    ├── Auth/ (认证页面)
    ├── System/ (系统管理)
    └── Business/ (业务模块)
        ↓
router (路由层)
    ├── index.tsx (路由配置)
    └── ProtectedRoute.tsx (路由保护)
```

### 4.2 状态管理架构

#### 4.2.1 职责分离的Store设计

```
┌─────────────────────────────────────────┐
│          authStore.ts                    │
│  (纯认证逻辑)                            │
│  ┌───────────────────────────────────┐  │
│  │ • token                           │  │
│  │ • isAuthenticated                 │  │
│  │ • loading                         │  │
│  │ • login()                         │  │
│  │ • logout()                        │  │
│  │ • clearAuthState()                │  │
│  └───────────────────────────────────┘  │
└─────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────┐
│          userStore.ts                    │
│  (用户信息 + 会话管理)                   │
│  ┌───────────────────────────────────┐  │
│  │ • user                            │  │
│  │ • session                         │  │
│  │   ├─ hasSelectedOrgRole           │  │
│  │   ├─ orgRoleData                  │  │
│  │   └─ loginStep                    │  │
│  │ • setUser()                       │  │
│  │ • refreshUserInfo()               │  │
│  │ • completeOrgRoleSelection()      │  │
│  └───────────────────────────────────┘  │
└─────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────┐
│          useAuth.ts                      │
│  (组合Hook - 协调各Store)                │
│  ┌───────────────────────────────────┐  │
│  │ const auth = useAuthStore()       │  │
│  │ const user = useUserStore()       │  │
│  │                                   │  │
│  │ return {                          │  │
│  │   auth, user,                     │  │
│  │   login, logout,                  │  │
│  │   isAuthenticated,                │  │
│  │   currentUser                     │  │
│  │ }                                 │  │
│  └───────────────────────────────────┘  │
└─────────────────────────────────────────┘
```

**设计优势:**
- ✅ 职责单一：认证、用户、会话各司其职
- ✅ 防重复操作：loading标志防止并发调用
- ✅ 原生localStorage：简化存储，移除加密复杂性
- ✅ 自动状态恢复：Zustand persist自动从localStorage恢复

#### 4.2.2 登录流程

```
用户提交登录表单
    ↓
1. 调用 useAuth().login(credentials)
   ├─ 设置 loginStep = 'initial'
   └─ 调用 authStore.login()
       ├─ API请求: POST /api/auth/login
       ├─ 保存 token 到 authStore
       ├─ 启动 tokenManager
       └─ 启动 sessionManager
   ↓
2. 设置 loginStep = 'authenticated'
   ├─ 显示机构角色选择页面
   └─ 用户选择机构和角色
   ↓
3. 调用 user.completeOrgRoleSelection(userDetails)
   ├─ 保存 orgRoleData 到 userStore
   ├─ 设置 loginStep = 'completed'
   └─ 设置 hasSelectedOrgRole = true
   ↓
4. 路由守卫检查通过
   └─ 重定向到首页 /home
```

### 4.3 模块化Layout系统

#### 4.3.1 三层Layout架构

```
┌──────────────────────────────────────────┐
│       BasicLayout.tsx (容器层)            │
│  ┌────────────────────────────────────┐  │
│  │ 用户状态验证                        │  │
│  │ Loading展示                        │  │
│  │ 43行代码 (优化60%)                 │  │
│  └────────────────────────────────────┘  │
└──────────────────────────────────────────┘
                  ↓
┌──────────────────────────────────────────┐
│    LayoutProvider.tsx (状态层)            │
│  ┌────────────────────────────────────┐  │
│  │ • 侧边栏状态 (collapsed)           │  │
│  │ • Tab列表和激活状态                │  │
│  │ • 路径映射 (基于用户菜单)          │  │
│  │ • 页面刷新控制                     │  │
│  │ • Tab操作方法 (9个)                │  │
│  │ • 防重复操作机制                   │  │
│  └────────────────────────────────────┘  │
└──────────────────────────────────────────┘
                  ↓
┌──────────────────────────────────────────┐
│   LayoutStructure.tsx (展示层)            │
│  ┌────────────────────────────────────┐  │
│  │ Sidebar (侧边栏)                   │  │
│  │ Header (顶部导航)                  │  │
│  │ TabSystem (标签页)                 │  │
│  │ Content (页面内容)                 │  │
│  │ Footer (底部)                      │  │
│  └────────────────────────────────────┘  │
└──────────────────────────────────────────┘
```

**优化成果:**
- BasicLayout: 107行 → 43行 (-60%)
- Hook数量: 4个 → 1个 (-75%)
- 职责更清晰，可维护性更强

#### 4.3.2 智能Tab系统

**Tab状态管理:**

```typescript
interface TabState {
  activeTabKey: string;          // 当前激活Tab
  tabList: TabItem[];            // Tab列表

  // Tab操作 (9个方法)
  addTab: (path: string, forceRefresh?: boolean) => void;
  switchTab: (key: string) => void;
  removeTab: (key: string) => void;
  refreshTab: (key: string) => void;
  closeLeftTabs: (currentKey: string) => void;
  closeRightTabs: (currentKey: string) => void;
  closeOtherTabs: (currentKey: string) => void;
}
```

**防重复操作机制:**

```typescript
// 使用ref标志防止并发操作
const isOperatingRef = useRef(false);

const switchTab = useCallback((targetKey: string) => {
  if (isOperatingRef.current) return;  // 防止重复点击

  isOperatingRef.current = true;
  // ... 执行切换逻辑

  setTimeout(() => {
    isOperatingRef.current = false;
  }, 100);
}, []);
```

**持久化策略:**
- localStorage存储Tab列表和激活状态
- 页面刷新后自动恢复Tab状态
- 首页Tab始终存在且不可关闭

### 4.4 动态路由与权限系统

#### 4.4.1 四层安全防护

```
┌─────────────────────────────────────────┐
│  ProtectedRoute.tsx (路由保护组件)       │
│                                         │
│  1️⃣ 基础认证检查 (快速响应)              │
│     if (!isAuthenticated) → /login    │
│                                         │
│  2️⃣ 机构角色检查                         │
│     if (!hasSelectedOrgRole) → /login │
│                                         │
│  3️⃣ 用户状态验证中                       │
│     if (loading) → PageLoading        │
│                                         │
│  4️⃣ 用户状态无效                         │
│     if (!userStatus.isValid) → /login │
│                                         │
│  ✅ 所有检查通过 → 渲染页面               │
└─────────────────────────────────────────┘
```

#### 4.4.2 动态页面加载流程

```
用户访问 /system/menu
    ↓
DynamicPage组件
    ↓
1. 构建权限索引 (O(1)查询优化)
   permissionPaths = useMemo(() => {
     const paths = new Set<string>();
     // 递归遍历menuTrees，收集所有路径
     return paths;
   }, [currentUser?.menuTrees]);
   ↓
2. O(1)权限检查
   hasPermission = permissionPaths.has('/system/menu')
   if (!hasPermission) return <NotFoundPage />
   ↓
3. 构建组件路径
   /system/menu → ../../pages/System/Menu/index.tsx
   ↓
4. 动态加载组件
   const PageComponent = lazy(() => import(componentPath))
   ↓
5. 渲染组件 (带ErrorBoundary)
   <Suspense fallback={<PageLoading />}>
     <ErrorBoundary>
       <PageComponent />
     </ErrorBoundary>
   </Suspense>
```

**性能优化:**

| 优化项 | 优化前 | 优化后 | 提升 |
|--------|--------|--------|------|
| 权限检查复杂度 | O(n) 递归遍历 | O(1) Set查询 | 100x+ |
| 组件加载方式 | 静态导入 | 懒加载 | 减少首屏体积 |
| 重复渲染 | 每次都计算 | useMemo缓存 | 避免重复计算 |

---

## 五、安全架构

### 5.1 多层安全防护

```
┌────────────────────────────────────────────────────┐
│                 客户端 (SVT-Web)                    │
└────────────────────────────────────────────────────┘
                        │
                 HTTPS + AES-256加密
                        ↓
┌────────────────────────────────────────────────────┐
│              过滤器链 (Filter Chain)                │
│  ┌──────────────────────────────────────────────┐  │
│  │ 1. AESCryptoFilter                           │  │
│  │    ├─ 检查 X-Encrypted: true                │  │
│  │    ├─ AES-256解密请求体                      │  │
│  │    └─ AES-256加密响应体                      │  │
│  └──────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────┐  │
│  │ 2. JwtAuthenticationFilter                   │  │
│  │    └─ 九步安全检查                           │  │
│  └──────────────────────────────────────────────┘  │
└────────────────────────────────────────────────────┘
                        │
┌────────────────────────────────────────────────────┐
│                AOP切面 (Aspect)                     │
│  ┌──────────────────────────────────────────────┐  │
│  │ PermissionAspect                             │  │
│  │ └─ @RequiresPermission权限验证               │  │
│  └──────────────────────────────────────────────┘  │
└────────────────────────────────────────────────────┘
                        │
┌────────────────────────────────────────────────────┐
│               业务逻辑层 (Service)                  │
└────────────────────────────────────────────────────┘
```

### 5.2 加密体系

#### 5.2.1 三层加密策略

| 加密类型 | 算法 | 用途 | 密钥管理 |
|---------|------|------|----------|
| **API加密** | AES-256-CBC | 请求/响应体加密 | 环境变量 `SVT_AES_KEY` |
| **配置加密** | SM4 | 配置文件敏感信息 | 环境变量 `SM4_ENCRYPTION_KEY` |
| **密码哈希** | Argon2 | 用户密码存储 | 内置参数配置 |

#### 5.2.2 AES-256加密流程

**请求加密 (前端):**

```typescript
// 1. 生成随机IV
const iv = CryptoJS.lib.WordArray.random(16);

// 2. AES-256-CBC加密
const encrypted = CryptoJS.AES.encrypt(
  JSON.stringify(data),
  CryptoJS.enc.Utf8.parse(aesKey),
  {
    iv: iv,
    mode: CryptoJS.mode.CBC,
    padding: CryptoJS.pad.Pkcs7
  }
);

// 3. 构造加密请求
const encryptedRequest = {
  encrypted: true,
  data: encrypted.toString(),  // Base64编码
  iv: iv.toString(CryptoJS.enc.Hex)
};

// 4. 发送请求
fetch('/api/xxx', {
  method: 'POST',
  headers: { 'X-Encrypted': 'true' },
  body: JSON.stringify(encryptedRequest)
});
```

**请求解密 (后端):**

```java
// 1. 检查请求头
if ("true".equals(request.getHeader("X-Encrypted"))) {
    // 2. 解析加密数据
    Map<String, Object> encryptedData = objectMapper.readValue(body, Map.class);
    String encryptedText = (String) encryptedData.get("data");
    String ivHex = (String) encryptedData.get("iv");

    // 3. AES-256解密
    byte[] iv = Hex.decode(ivHex);
    byte[] encrypted = Base64.getDecoder().decode(encryptedText);

    Cipher cipher = Cipher.getInstance("AES/CBC/PKCS5Padding");
    cipher.init(Cipher.DECRYPT_MODE, secretKeySpec, new IvParameterSpec(iv));
    byte[] decrypted = cipher.doFinal(encrypted);

    // 4. 返回明文JSON
    return new String(decrypted, StandardCharsets.UTF_8);
}
```

#### 5.2.3 Argon2密码哈希

**配置参数:**

```java
@Configuration
public class SVTArgon2PasswordEncoder extends Argon2PasswordEncoder {
    public SVTArgon2PasswordEncoder() {
        super(
            16,     // saltLength: 盐长度16字节
            32,     // hashLength: 哈希长度32字节
            1,      // parallelism: 并行度1
            65536,  // memory: 内存开销64MB
            3       // iterations: 迭代次数3
        );
    }
}
```

**哈希示例:**

```
原始密码: admin123

Argon2哈希:
$argon2id$v=19$m=65536,t=3,p=1$
abc123def456xyz789$
1a2b3c4d5e6f7g8h9i0j...
```

**安全优势:**
- ✅ 抗GPU/ASIC攻击 (内存密集型)
- ✅ 抗并行攻击 (可配置并行度)
- ✅ 抗彩虹表攻击 (随机盐)

### 5.3 JWT智能续期机制

**活跃度续期参数:**

```yaml
svt:
  jwt:
    activity-cycle-seconds: 600        # 活跃度周期10分钟
    activity-renewal-threshold: 20     # 续期阈值20% (最后2分钟)
```

**续期逻辑:**

```
用户最后活动时间: 10:00:00
活跃度周期: 10分钟
续期窗口: 最后20% (最后2分钟)

时间线:
10:00:00 ─────────── 10:08:00 ─────────── 10:10:00
         正常期                续期窗口          过期

在续期窗口内的任何请求 → 自动续期
    更新 lastActivityTime
    更新 activityCycleStartTime
    重新计算过期时间
```

**会话状态响应头:**

```
X-Session-Status: NORMAL / EXPIRED
X-Session-Remaining: 120000 (毫秒)
X-Session-Warning: JWT_TOKEN_EXPIRED / ACTIVITY_EXPIRED
```

---

## 六、数据架构

### 6.1 数据库设计

#### 6.1.1 核心表结构

```
┌─────────────────────────────────────┐
│        系统管理表                    │
├─────────────────────────────────────┤
│ user_info        用户表              │
│ role_info        角色表              │
│ menu_info        菜单表              │
│ org_info         组织机构表          │
│ user_role        用户角色关联表      │
│ role_menu        角色菜单关联表      │
│ user_org         用户组织关联表      │
└─────────────────────────────────────┘

┌─────────────────────────────────────┐
│        审计日志表                    │
├─────────────────────────────────────┤
│ audit_log        审计日志表          │
└─────────────────────────────────────┘

┌─────────────────────────────────────┐
│        框架表                        │
├─────────────────────────────────────┤
│ db_key           分布式ID表          │
│ distributed_lock 分布式锁表          │
└─────────────────────────────────────┘
```

#### 6.1.2 标准字段规范

**所有表必须包含的字段:**

```sql
-- 审计字段 (必须)
create_by VARCHAR(32) COMMENT '创建者',
create_time DATETIME DEFAULT GETDATE() COMMENT '创建时间',
update_by VARCHAR(32) COMMENT '更新者',
update_time DATETIME COMMENT '更新时间',

-- 逻辑删除 (必须)
del_flag CHAR(1) DEFAULT '0' COMMENT '删除标志 (0正常 1删除)'
```

**命名规范:**
- 表名：小写+下划线 (user_info)
- 字段名：小写+下划线 (user_id)
- 主键约束：pk_表名 (pk_user_info)
- 索引名：idx_表名_字段名 (idx_user_info_login_id)

### 6.2 分布式ID设计

**ID格式:** `前缀 + 日期 + 序号 + 字母扩展`

```
U20250617000001    # 用户ID
M20250617000002    # 菜单ID
R20250617000003A   # 角色ID (序号超999999后加字母)
```

**db_key表结构:**

```sql
CREATE TABLE db_key (
    table_name VARCHAR(50) PRIMARY KEY,      -- 表名
    field_name VARCHAR(50) NOT NULL,         -- 字段名
    prefix VARCHAR(10),                      -- 前缀 (U, M, R)
    current_id BIGINT DEFAULT 1,             -- 当前序号
    current_letter_position INT DEFAULT 0,   -- 当前字母位置
    record_date DATETIME,                    -- 记录日期
    step INT DEFAULT 100,                    -- 步长 (批量生成)
    create_time DATETIME DEFAULT GETDATE(),
    update_time DATETIME
);
```

**日期重置机制:**
- 每天凌晨自动重置序号为1
- 确保ID按日期递增

### 6.3 分布式锁设计

**distributed_lock表结构:**

```sql
CREATE TABLE distributed_lock (
    lock_key VARCHAR(255) PRIMARY KEY,       -- 锁键 (唯一约束保证互斥)
    lock_value VARCHAR(255),                 -- 锁值 (UUID)
    holder_info VARCHAR(500),                -- 持有者信息
    created_time DATETIME,                   -- 创建时间
    expire_time DATETIME,                    -- 过期时间
    retry_count INT DEFAULT 0                -- 重试次数
);
```

**获取锁流程:**
1. 尝试插入记录 (依赖主键唯一性)
2. 插入成功 → 获取锁成功
3. 主键冲突 → 获取锁失败 → 重试

**释放锁流程:**
1. 验证 lock_value 匹配 (防止误释放)
2. 删除锁记录

**定时清理任务:**
- 每60秒清理过期锁
- 清理达到最大重试次数的锁

---

## 七、部署架构

### 7.1 容器化部署

```yaml
# docker-compose.yml
version: '3.8'

services:
  # 后端服务
  svt-server:
    image: svt-server:1.0.0
    container_name: svt-server
    ports:
      - "8080:8080"
    environment:
      - SPRING_PROFILES_ACTIVE=prod
      - SM4_ENCRYPTION_KEY=${SM4_KEY}
      - SVT_AES_KEY=${AES_KEY}
    depends_on:
      - sqlserver
    networks:
      - svt-network

  # 前端服务
  svt-web:
    image: svt-web:1.0.0
    container_name: svt-web
    ports:
      - "80:80"
    networks:
      - svt-network

  # 数据库
  sqlserver:
    image: mcr.microsoft.com/mssql/server:2019-latest
    container_name: svt-sqlserver
    ports:
      - "1433:1433"
    environment:
      - ACCEPT_EULA=Y
      - SA_PASSWORD=${DB_PASSWORD}
    volumes:
      - sqlserver-data:/var/opt/mssql
    networks:
      - svt-network

volumes:
  sqlserver-data:

networks:
  svt-network:
    driver: bridge
```

### 7.2 Nginx配置

```nginx
upstream svt-backend {
    ip_hash;  # Session Sticky: 基于IP的会话粘性
    server svt-server-1:8080;
    server svt-server-2:8080;
}

server {
    listen 80;
    server_name svt.example.com;

    # 前端静态资源
    location / {
        root /usr/share/nginx/html;
        try_files $uri $uri/ /index.html;
    }

    # API代理
    location /api/ {
        proxy_pass http://svt-backend;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_connect_timeout 30s;
        proxy_send_timeout 30s;
        proxy_read_timeout 30s;
    }
}
```

**关键配置说明:**
- ✅ **ip_hash**: 确保同一IP的请求路由到同一后端实例
- ✅ **try_files**: 支持前端路由History模式
- ✅ **proxy_timeout**: 防止长时间请求超时

### 7.3 多环境配置

```
环境列表:
├── dev (开发环境)
│   ├── application-dev.yml
│   ├── .env.development
│   └── 本地数据库
│
├── uat (UAT环境)
│   ├── application-uat.yml
│   ├── .env.uat
│   └── UAT数据库
│
└── prod (生产环境)
    ├── application-prod.yml
    ├── .env.production
    └── 生产数据库
```

**环境变量管理:**

```bash
# 后端环境变量
export SPRING_PROFILES_ACTIVE=prod
export SM4_ENCRYPTION_KEY=xxx
export SVT_AES_KEY=xxx
export DB_PASSWORD=xxx

# 前端环境变量
export VITE_API_BASE_URL=https://api.svt.example.com
export VITE_AES_KEY=xxx
```

---

## 八、性能优化

### 8.1 后端性能优化

#### 8.1.1 数据库优化

**索引优化:**

```sql
-- 查询频繁字段建立索引
CREATE INDEX idx_user_info_login_id ON user_info(login_id);
CREATE INDEX idx_user_info_status ON user_info(status);
CREATE INDEX idx_audit_log_create_time ON audit_log(create_time);

-- 复合索引 (最左前缀原则)
CREATE INDEX idx_user_info_status_create_time
ON user_info(status, create_time);
```

**批量操作:**

```java
// ✅ 批量插入
userInfoMapper.insertBatch(userList);

// ❌ 循环插入 (N次数据库访问)
for (UserInfo user : userList) {
    userInfoMapper.insert(user);
}
```

**分页查询:**

```java
// 使用MyBatis-Flex分页
Page<UserInfo> page = userInfoMapper.paginate(
    pageNumber,
    pageSize,
    QueryWrapper.create().from(USER_INFO)
);
```

#### 8.1.2 缓存优化

**缓存使用策略:**

```java
// 读操作：先查缓存
UserDetailCache cached = UserDetailCacheUtils.get(userId);
if (cached != null) {
    return cached;  // 缓存命中
}

// 缓存未命中，查询数据库
UserInfo user = userInfoMapper.selectById(userId);
UserDetailCache cache = convertToCache(user);

// 写入缓存
UserDetailCacheUtils.put(userId, cache);
return cache;
```

**缓存失效策略:**

```java
// 更新/删除时清除缓存
@CacheEvict(value = "user", key = "#userId")
public void updateUser(String userId, UserInfo userInfo) {
    userInfoMapper.updateById(userInfo);
}
```

### 8.2 前端性能优化

#### 8.2.1 代码分割

**路由级懒加载:**

```typescript
// 懒加载页面组件
const LoginPage = lazy(() => import('@/pages/Auth/LoginPage'));
const HomePage = lazy(() => import('@/pages/Home/HomePage'));

// 动态页面模块懒加载
const pageModules = import.meta.glob('../../pages/**/index.tsx');
```

**Vite手动分包:**

```typescript
// vite.config.ts
build: {
  rollupOptions: {
    output: {
      manualChunks: {
        vendor: ['react', 'react-dom'],      // 520KB → 180KB
        antd: ['antd', '@ant-design/icons'], // → 200KB
        router: ['react-router-dom'],        // → 40KB
        utils: ['axios', 'dayjs', 'crypto-js'] // → 100KB
      }
    }
  }
}
```

**优化效果:**

| 指标 | 优化前 | 优化后 | 提升 |
|------|--------|--------|------|
| Bundle大小 | 850KB | 520KB | -39% |
| 首屏加载时间 | 2.5s | 1.2s | -52% |
| FCP | 1.5s | 0.8s | -47% |

#### 8.2.2 React优化

**useMemo缓存计算结果:**

```typescript
// 权限索引构建 (O(1)查询)
const permissionPaths = useMemo(() => {
  return buildPermissionIndex(currentUser?.menuTrees || []);
}, [currentUser?.menuTrees]);
```

**useCallback缓存函数引用:**

```typescript
// 事件处理函数
const handleMenuClick = useCallback((key: string) => {
  addTab(key, false);
}, [addTab]);
```

**组件级优化:**

```typescript
// React.memo防止不必要的重渲染
const UserCard = React.memo<UserCardProps>(({ user, onClick }) => {
  return <div onClick={() => onClick(user)}>{user.name}</div>;
});
```

#### 8.2.3 网络优化

**请求优化:**

```typescript
// GET请求防缓存
if (config.method === 'get') {
  config.params = { ...config.params, _t: Date.now() };
}

// 请求超时设置
const request = axios.create({
  timeout: 15000  // 15秒超时
});
```

**并行请求:**

```typescript
// 并行请求多个API
const [users, roles, menus] = await Promise.all([
  userApi.getUserList(),
  roleApi.getRoleList(),
  menuApi.getMenuTree()
]);
```

---

## 九、技术债务

### 9.1 已知问题

| 问题 | 严重程度 | 影响范围 | 优先级 | 建议解决方案 |
|------|----------|----------|--------|--------------|
| **缺少单元测试** | 高 | 全局 | P0 | 补充测试覆盖率到80% |
| **缺少E2E测试** | 高 | 全局 | P0 | 引入Playwright/Cypress |
| **API层缺少重试机制** | 中 | 网络层 | P1 | 添加指数退避重试 |
| **缺少性能监控** | 中 | 全局 | P1 | 集成APM工具 (Sentry) |
| **部分组件缺少PropTypes** | 低 | 局部 | P2 | 完善TypeScript类型 |

### 9.2 改进建议

#### 9.2.1 短期改进 (1-3个月)

**1. 测试覆盖率提升**

```typescript
// 添加单元测试
describe('useAuth', () => {
  it('should login successfully', async () => {
    const { result } = renderHook(() => useAuth());
    await act(() => result.current.login({
      loginId: 'test',
      password: '123'
    }));
    expect(result.current.isAuthenticated).toBe(true);
  });
});
```

**2. 性能监控集成**

```typescript
// 集成Sentry
import * as Sentry from "@sentry/react";

Sentry.init({
  dsn: "your-dsn",
  integrations: [new Sentry.BrowserTracing()],
  tracesSampleRate: 1.0
});
```

**3. API重试机制**

```typescript
// Axios拦截器添加重试
axios.interceptors.response.use(null, async (error) => {
  const config = error.config;

  if (!config || !config.retry) {
    return Promise.reject(error);
  }

  config.retryCount = config.retryCount || 0;

  if (config.retryCount >= config.retry) {
    return Promise.reject(error);
  }

  config.retryCount += 1;
  const backoff = new Promise(resolve => {
    setTimeout(() => resolve(null), config.retryDelay || 1000);
  });

  await backoff;
  return axios(config);
});
```

#### 9.2.2 中期改进 (3-6个月)

**1. 微前端架构探索**

```
主应用 (SVT-Web)
├── 系统管理微应用
├── 业务管理微应用
└── 报表分析微应用
```

**2. PWA支持**

```typescript
// Service Worker注册
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.register('/sw.js');
}
```

**3. 国际化支持**

```typescript
import i18n from 'i18next';

i18n.init({
  resources: {
    en: { translation: { /* English */ } },
    zh: { translation: { /* Chinese */ } }
  }
});
```

---

## 十、未来规划

### 10.1 技术演进路线

```
当前版本 v1.0.0
    ↓
短期 (3-6个月)
    ├─ 测试覆盖率 → 80%
    ├─ 性能监控集成
    └─ API重试机制
    ↓
中期 (6-12个月)
    ├─ 微前端架构
    ├─ PWA支持
    └─ 国际化支持
    ↓
长期 (12-24个月)
    ├─ GraphQL替代REST
    ├─ Kubernetes集群部署
    └─ AI辅助开发
```

### 10.2 架构演进方向

**1. 从单体到微服务**

```
当前: 单体Spring Boot应用
    ↓
未来: 微服务架构
    ├── 用户服务 (user-service)
    ├── 权限服务 (auth-service)
    ├── 审计服务 (audit-service)
    └── 网关服务 (gateway-service)
```

**2. 从本地缓存到分布式缓存**

```
当前: Caffeine本地缓存
    ↓
未来: Redis分布式缓存 (高并发场景)
```

**3. 从关系型数据库到混合存储**

```
当前: SQL Server (关系型)
    ↓
未来: SQL Server + Elasticsearch (搜索) + MongoDB (文档)
```

### 10.3 关键指标目标

| 指标 | 当前 | 短期目标 | 长期目标 |
|------|------|----------|----------|
| **测试覆盖率** | 0% | 80% | 90% |
| **首屏加载时间** | 1.2s | 0.8s | 0.5s |
| **API响应时间(P95)** | 200ms | 150ms | 100ms |
| **并发用户数** | 500 | 2000 | 5000 |
| **可用性** | 99.5% | 99.9% | 99.99% |

---

## 十一、附录

### 11.1 关键文件索引

#### 后端关键文件

| 功能 | 文件路径 | 行数 |
|------|----------|------|
| 应用入口 | `SvtApplication.java` | 50 |
| JWT认证过滤器 | `frame/security/filter/JwtAuthenticationFilter.java` | 300 |
| AES加密过滤器 | `common/filter/AESCryptoFilter.java` | 200 |
| 分布式锁 | `frame/lock/DatabaseDistributedLockManager.java` | 250 |
| 分布式ID | `frame/dbkey/DistributedIdGenerator.java` | 200 |

#### 前端关键文件

| 功能 | 文件路径 | 行数 |
|------|----------|------|
| 应用入口 | `main.tsx` | 30 |
| 路由配置 | `router/index.tsx` | 100 |
| 认证Store | `stores/authStore.ts` | 150 |
| HTTP客户端 | `utils/request.ts` | 200 |
| 布局Provider | `components/Layout/core/LayoutProvider.tsx` | 300 |

### 11.2 API接口清单

详见 Knife4j文档: http://localhost:8080/doc.html

**核心接口:**
- `POST /api/auth/login` - 用户登录
- `GET /api/auth/logout` - 用户登出
- `POST /api/auth/verify-user-status` - 用户状态验证
- `POST /api/system/menu/get-all-menu-tree` - 获取菜单树
- `POST /api/system/user/list` - 获取用户列表

### 11.3 更新日志

| 日期 | 版本 | 更新内容 | 作者 |
|------|------|----------|------|
| 2025-11-17 | 1.0.0 | 初始版本，完整系统架构文档 | Winston (Architect) |

---

## 文档维护

**维护周期**: 每季度审查一次，重大架构变更时及时更新
**最后更新**: 2025-11-17
**负责人**: Winston (System Architect)
**联系方式**: architect@svt.example.com

---

**© 2025 SVT Project. All rights reserved.**
