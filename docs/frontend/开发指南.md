...
- **前端框架**: React 19 + TypeScript 5.8+ + Vite 6.3+
- **UI组件库**: Ant Design 5.25+ + @ant-design/icons
- **状态管理**: Zustand 5.0+ + TanStack Query 5.80+
- **路由管理**: React Router 7.6+
- **表单处理**: React Hook Form 7.57+ + Zod 3.25+
- **HTTP客户端**: Axios 1.9+
- **加密处理**: crypto-js 4.2+ (AES加密)

## 🎯 设计原则

### 架构设计理念

- **类型优先**: 全面的TypeScript类型覆盖，编译时错误检查
- **模块化架构**: 清晰的模块边界和职责分离
- **性能优化**: 代码分割、懒加载、缓存策略
- **安全设计**: 多层安全防护和数据加密
- **开发体验**: 完善的调试工具和开发规范
...
export const useAuthStore = create<AuthState>()(
  persist(
    ...
    {
      name: 'auth-storage',
      storage: createEncryptedStorage(), // 统一存储适配器
      partialize: (state) => ({
        token: state.token,
...
VITE_AES_ENABLED=true  # 启用加密
VITE_DEBUG_MODE=true   # 开发环境调试
```
...
详细文档：[存储管理指南](./Storage-Management.md)

## 🐛 调试管理

### DebugManager 使用

SVT提供统一的调试信息管理工具，支持分级调试和环境隔离：

```typescript
import { DebugManager } from '@/utils/debugManager';

// 基础调试信息
DebugManager.log('Component', '组件已挂载', { props });

// 错误信息
DebugManager.error('API', '请求失败', error);

// 性能监控
DebugManager.performance('API Call', startTime, endTime);

// 敏感信息（仅开发环境）
DebugManager.sensitive('Auth', 'Token信息', tokenData);
```
...
浏览器控制台可用的调试方法：
- `debugLocalStorage()` - 查看存储状态
- `testStorage()` - 测试存储功能
- `clearAllStorage()` - 清理所有存储
